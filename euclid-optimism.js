  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        // Dynamically set maxFeePerGas and maxPriorityFeePerGas for Optimism
        try {
...
const checkSwapStatus = async (txHash, chainUid, walletAddress, provider, logger, meta, maxAttempts = 10, delay = 30000) => {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (SHOW_SWAP_PENDING_LOG.optimism) {
        logger.loading(`[Optimism Sepolia] Swap pending (Attempt ${attempt}/10): ${txHash}`);
      }
    } catch (error) {
