  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    while (attempt < maxAttempts) {
      try {
        ...
        // Dynamically set maxFeePerGas and maxPriorityFeePerGas for Soneium
        try {
...
// Helper: checkSwapStatus (Soneium swap pending log)
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (SHOW_SWAP_PENDING_LOG.soneium) {
        logger.loading(`[Soneium Minato] Swap pending (Attempt ${attempt}/10): ${txHash}`);
      }
    } catch (error) {
